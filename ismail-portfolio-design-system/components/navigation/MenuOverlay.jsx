...
export function MenuOverlay({
  open = false, onClose, name = "Ismail Kayadelen",
  items = [{ label: "Homepage", href: "#home" }, { label: "About me", href: "#about" }, { label: "Work", href: "#work" }, { label: "Blog", href: "#blog" }, { label: "Contact", href: "#contact" }],
  socials = [{ label: "Dribbble", href: "#" }, { label: "Behance", href: "#" }, { label: "Instagram", href: "#" }],
  email = "ismail@kayadelen.studio", onNavigate, style, ...rest
}) {
    ...
    <div role="dialog" aria-modal="true" aria-hidden={!open} style={{
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <button onClick={onClose} aria-label="Close menu" style={{
          ...
          border: "1px solid var(--ev-border-inverse)", background: "transparent",
        ...
        {items.map((it, i) => (
          <a key={it.label} href={it.href}
            ...
            style={{
              ...
              fontSize: "clamp(34px, 6.4vw, 92px)", lineHeight: 1.02, letterSpacing: "var(--ev-ls-display)",
            ...
            <span style={{ fontFamily: "var(--ev-font-mono)", fontSize: "var(--ev-size-label)", letterSpacing: "var(--ev-ls-label)", opacity: .5, fontWeight: 400 }}>
              0{i + 1}
            </span>
            {it.label}
          </a>
        ...
        <div style={{ display: "flex", gap: "var(--ev-space-5)", flexWrap: "wrap" }}>
          {socials.map((s) => (
            <a key={s.label} href={s.href} style={{
              ...
              textTransform: "uppercase", letterSpacing: "var(--ev-ls-label)", color: "var(--ev-paper)"
            }}>{s.label}</a>
          ))}
        </div>
        <Eyebrow dot="signal" inverse>{email}</Eyebrow>
      </div>
    </div>
  );
}
